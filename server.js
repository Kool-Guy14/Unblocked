...
function clean(u){return {username:u.username,createdAt:u.createdAt,keyRedeemed:!!u.keyRedeemed,redeemedKey:u.redeemedKey||null,keyCreatedAt:u.keyCreatedAt||null,keyExpiresAt:u.keyExpiresAt||null,inventory:u.inventory||[],stats:u.stats||{plinkoBest:0,lockBest:0},cookieData:u.cookieData||{count:0,perClick:1}}}
...
function route(req,res){
 ...
 if(u.pathname==="/api/admin/key"&&req.method==="POST")return body(req).then(b=>{let ds={hour:3600000,day:86400000,week:604800000,month:2592000000,permanent:0},d=ds[b.duration];if(d===undefined)return json(res,400,{error:"Invalid duration"});let key=b.key&&String(b.key).trim();if(key){if(db.keys[key])return json(res,400,{error:"Key already exists"})}else{key="ULTRA-"+crypto.randomBytes(2).toString("hex").toUpperCase()+"-"+crypto.randomBytes(2).toString("hex").toUpperCase()}db.keys[key]={createdAt:Date.now(),expiresAt:d?Date.now()+d:null,used:false,redeemedBy:null,redeemedAt:null};save();json(res,200,{key,...db.keys[key]})})
 ...
 if(u.pathname==="/api/roll"&&req.method==="POST"){let user=auth(req);if(!active(user))return json(res,403,{error:"Active key required"});if((user.inventory||[]).length>=MAX_INV)return json(res,400,{error:"Inventory full (max 10). Fuse some brainrots first."});let item=roll();user.inventory.push(item);save();return json(res,200,{item,user:clean(user),luck:weights().luck})}
 ...
 if(u.pathname==="/api/stats"&&req.method==="POST")return body(req).then(b=>{let user=auth(req);if(!user)return json(res,401,{error:"Login required"});let game=b.game,score=Number(b.score)||0;if(!["plinko","lock"].includes(game))return json(res,400,{error:"Invalid game"});user.stats=user.stats||{plinkoBest:0,lockBest:0};let key=game+"Best";if(score>(user.stats[key]||0))user.stats[key]=score;save();json(res,200,{user:clean(user)})})
 if(u.pathname==="/api/game/cookie"&&req.method==="POST")return body(req).then(b=>{let user=auth(req);if(!user)return json(res,401,{error:"Login required"});user.cookieData={count:Math.max(0,Number(b.count)||0),perClick:Math.max(1,Number(b.perClick)||1)};save();json(res,200,{user:clean(user)})})
 ...
 if(u.pathname==="/bookmarklet.js"){res.writeHead(200,{"Content-Type":"application/javascript","Cache-Control":"no-store","Access-Control-Allow-Origin":"*","Access-Control-Allow-Methods":"GET, POST, OPTIONS","Access-Control-Allow-Headers":"Content-Type, Authorization"});return res.end(fs.readFileSync(path.join(PUBLIC,"bookmarklet.js"),"utf8").replace(/^javascript:/,""))}
